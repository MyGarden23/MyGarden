...
/**
 * Simple unit test that tests the Firestore Mapper object behaviour.
 * They test that plants/owedPlants are the same when serialized then deserialized. */
class FirestoreMapperTests {

    private val recognizedPlant = Plant(
        name = "Rose",
        image = null,
        latinName = "Rosalia",
    ...
    private val ownedPlantRecognized = OwnedPlant(
        id = "1",
        plant = recognizedPlant,
        lastWatered = Timestamp(System.currentTimeMillis())
    )

    private val ownedPlantUnrecognized = OwnedPlant(
        id = "2",
