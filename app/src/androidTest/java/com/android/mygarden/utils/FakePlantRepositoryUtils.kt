 ...
 *
 * This enum is used to specify which repository implementation should be
 * instantiated and spied upon in [FakePlantRepositoryUtils].
 */
 ...
 *
 * This class provides a spy wrapper around either [PlantsRepositoryLocal] or [PlantsRepositoryFirestore]
 * and helper methods to mock API responses for testing purposes. It allows tests to control the behavior
 * of repository methods without making actual network calls.
 *
 * ## Usage Example
 * ```kotlin
 ...
 * ```
 *
 * @param repoType The type of repository to create and spy upon, specified using [PlantRepositoryType]
 * @property repository A MockK spy of the selected repository implementation ([PlantsRepositoryLocal]
 *                      or [PlantsRepositoryFirestore]) that can be configured to return predetermined
 *                      responses for testing scenarios.
 * @throws IllegalArgumentException If an unsupported repository type is provided (should never occur
 *                                  with the current enum values)
 */
class FakePlantRepositoryUtils(repoType: PlantRepositoryType)  {
    private val repository: PlantsRepository = when(repoType) {
     ...
     *
     * This mock response represents a successful plant identification for a tomato plant
     * (Solanum lycopersicum L.). The response includes:
     ...
     * useful for integration tests where multiple components need to interact with the same
     * mocked repository instance.
     *
     * **Important:** This method should be called in the test setup (e.g., `@Before` method) to ensure
     * the mock repository is properly configured before test execution. Consider resetting the
     ...
     * [PlantsRepository.identifyPlant] is called. The path parameter passed to identifyPlant
     * will be automatically captured and used in the returned Plant's image field.
     *
     * @param plant The Plant object to return from the mocked API call. The image field
     *              will be overridden with the actual path parameter from the call.
     *
     ...
     * [PlantsRepository.plantDescriptionCallGemini] is called with any parameter. This allows
     * tests to verify behavior without making actual API calls to the Gemini service.
     *
     ...
     *
     * This method configures the repository spy to return the specified JSON string when
     * [PlantsRepository.plantNetAPICall] is called with any parameters. This enables testing
     * of plant identification features without requiring network connectivity or consuming
     * API quota.
     *
     * @param resJSON The JSON response string to return from the mocked API call
     * @return The result of the test coroutine execution
