...
/**
 * Authentication tests for MyGarden.
 * Mirrors the Bootcamp style: UI checks + emulator-backed auth, without production code changes.
 */
...
class AuthenticationTest {
    ...
    fun loggedOut_shows_signIn_screen() {
        ...
        // Replace with your button text/tag if different (e.g., use a testTag)
    ...
    fun can_sign_in_with_google_via_fake_credential_manager() {
        ...
        compose.onNodeWithText("Sign in").assertIsDisplayed().performClick()
    ...
    @Test
    fun can_sign_in_with_existing_account() = kotlinx.coroutines.runBlocking {
        // 1) Seed an existing user directly in the emulator
        val email = "existing@test.com"
        val idToken = FakeJwtGenerator.createFakeGoogleIdToken(name = "Existing User", email = email)
