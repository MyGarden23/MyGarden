...
/**
 * End-to-end tests for MyGarden application.
 * These tests assume the user is already signed in so we begin on the camera screen.
 */
@RunWith(AndroidJUnit4::class)
class EndToEndM1 {

    /**
     * Main compose test rule that creates and manages the MainActivity for testing.
     * Provides access to the full app UI hierarchy and interaction methods.
     */
    @get:Rule
    val composeTestRule = createAndroidComposeRule<MainActivity>()

    /**
     * Automatically grants camera permission before tests run.
     * Essential for camera functionality testing without user interaction prompts.
     */
    @get:Rule
    ...
    fun endToEndTest() {
        ...
        composeTestRule.onNodeWithTag(PlantInfoScreenTestTags.DESCRIPTION_TEXT).assertTextEquals("Roses are red")
        ...
        composeTestRule.onNodeWithTag(PlantInfoScreenTestTags.PLANT_LATIN_NAME).assertTextEquals("Rosum")
        ...
        composeTestRule.onNodeWithTag(PlantInfoScreenTestTags.HEALTH_STATUS_DESCRIPTION).assertTextEquals("The plant is healthy 🌱")
        composeTestRule.onNodeWithTag(PlantInfoScreenTestTags.HEALTH_STATUS).assertTextEquals("Status: The plant is healthy 🌱")
        composeTestRule.onNodeWithTag(PlantInfoScreenTestTags.WATERING_FREQUENCY).assertTextEquals("Watering Frequency: Every 2 days")
        ...
        // Future enhancement: Test profile screen navigation and functionality
    }
