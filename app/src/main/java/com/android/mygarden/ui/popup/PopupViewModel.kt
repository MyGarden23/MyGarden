...
class PopupViewModel(
  ...
  private val _thirstyPlants = MutableSharedFlow<OwnedPlant>(
    replay = 0,
  ...
  init {
    viewModelScope.launch {
      ...
      plantsRepo.plantsFlow.collect { list ->
        ...
        list
            // filter the list to keep only the plants that were not in status [NEEDS_WATER] but are now
            .filter { ownedPlant ->
              ...
              (ownedPlant.plant.healthStatus == PlantHealthStatus.NEEDS_WATER) && previousOwnedPlant?.plant?.healthStatus != PlantHealthStatus.NEEDS_WATER
            }
            // for each of these plants, emit it through the SharedFlow that will be collected by the UI to display a pop-up
            .forEach { becameThirsty ->
              _thirstyPlants.tryEmit(becameThirsty)
            }

        // update the previous collected list to the one just received for future collections of the Flow
